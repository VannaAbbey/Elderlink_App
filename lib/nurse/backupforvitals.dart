...
class UpcomingMedicationsTab extends StatefulWidget {
  ...
  // Optional callback to notify parent about the number of upcoming takes
...
class _UpcomingMedicationsTabState extends State<UpcomingMedicationsTab>
  ...
  Future<String?> _getNurseId() async {
    try {
      final rawName = (widget.nurseName ?? '').trim();
      if (rawName.isEmpty) return null;

      final nameParts = rawName.split(RegExp(r"\s+"));

      // Prefer exact first+last match when available
      ...
      // Fallback: try matching by first name alone, then last name alone
      final byFirst = await _firestore
          .collection('users')
          .where('user_fname', isEqualTo: rawName)
          .where('user_type', isEqualTo: 'nurse')
          .get();
      if (byFirst.docs.isNotEmpty) return byFirst.docs.first.id;

      final byLast = await _firestore
          .collection('users')
          .where('user_lname', isEqualTo: rawName)
          .where('user_type', isEqualTo: 'nurse')
          .get();
      if (byLast.docs.isNotEmpty) return byLast.docs.first.id;
  ...
  Future<void> _saveMedicationToDatabase({
    ...
    try {
      ...
      // Invalidate cache for current context so newly-created medication shows immediately
      try {
        final _currentDay = _getCurrentDay();
        final _currentShift = _getCurrentShift();
        final _cacheKey =
            '${widget.houseId}|${widget.nurseName ?? ''}|$_currentShift|$_currentDay';
        _medsCache.remove(_cacheKey);
        _medsCacheTime.remove(_cacheKey);
      } catch (_) {
        // If cache invalidation fails for any reason, continue to reload normally
      }

      // Refresh the medications list (force refresh)
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
                  ...
                  Column(
                    children: [
                      ...
                      Row(
                        mainAxisSize: MainAxisSize.min,
                        children: [
                          // Square check button for quick complete
                          IconButton(
                            icon: Icon(
                              status == 'complete'
                                  ? Icons.check_box
                            ...
                            onPressed: status == 'complete'
                                ...
                                : () async {
                                    ...
                                    await _updateTakeStatus(
                                      medication['id'],
                                      take['take_number'],
                                      'complete',
                                    );
                                    // Only play confetti if it changed to complete
            ...
            // Per-take delete UI intentionally removed. Use medication-level Delete All instead.
