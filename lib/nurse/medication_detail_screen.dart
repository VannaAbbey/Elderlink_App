...
import 'medication_activity_logger.dart';
...
class _MedicationDetailScreenState extends State<MedicationDetailScreen>
  ...
  Future<void> updateMedicationStatus(
    ...
    List<String> statuses = List<String>.filled(
      timesCount,
      "Pending",
      growable: true,
    );
    ...
    // Get medication name and house ID for logging
    String medicationName = data?['medication_name'] ?? 'Unknown';
    String houseId =
        'H001'; // Default house ID, will be updated from elderly data

    // Get house ID from elderly data
    try {
      ...
      if (elderlyDoc.exists) {
        houseId = elderlyDoc.data()?['house_id'] ?? 'H001';
      }
    } catch (e) {
      print('Error getting house ID: $e');
    }

    await MedicationActivityLogger.logStatusChange(
      medicationId: medicationId,
      elderlyId: widget.elderlyId,
      houseId: houseId,
      medicationName: medicationName,
      oldStatus:
          (data?['medication_status'] as List<dynamic>?)?[index]?.toString() ??
          'Pending',
      newStatus: status,
      takeNumber: index + 1,
      nurseName: nurseName,
  ...
  Future<void> saveMedication() async {
    ...
    await ref.set({
      ...
      "medication_status": List<String>.filled(
        draft.medTimes.length,
        "Pending",
      ), // initial
    ...
    // Get house ID from elderly data for logging
    String houseId = 'H001'; // Default house ID
    try {
      final elderlyDoc = await FirebaseFirestore.instance
          .collection('elderly')
          .doc(widget.elderlyId)
    ...
    if (editingMedId != null) {
      // Edit medication
      await MedicationActivityLogger.logEditMedication(
        medicationId: ref.id,
        elderlyId: widget.elderlyId,
        houseId: houseId,
        medicationName: draft.medNameController.text,
        nurseName: nurseName,
        oldData: oldData,
      ...
      // Add medication
      await MedicationActivityLogger.logAddMedication(
        medicationId: ref.id,
        elderlyId: widget.elderlyId,
        houseId: houseId,
        medicationName: draft.medNameController.text,
              ...
              children: [
                            ...
                            Expanded(
                              child: GestureDetector(
                                ...
                                child: Container(
                                  ...
                                  child: Text(
                                    draft.medTimes[i].format(context),
                                  ),
                                ),
                              ),
                            ),
                ...
                SizedBox(
                  ...
                  child: ElevatedButton(
                    onPressed: _allFieldsValid() && !isSaving
                        ? saveMedication
                        : null,
                    style: ElevatedButton.styleFrom(
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(
          "Medications - ${widget.elderlyName}",
          style: const TextStyle(
            fontWeight: FontWeight.bold,
            color: Color(0xFF00588E),
            fontSize: 18,
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              return Card(
                ...
                child: ExpansionTile(
                  title: Text(
                    medData["medication_name"] ?? "Unnamed",
                  ...
                  children: [
                    ...
                    for (var i = 0; i < times.length; i++)
                      ListTile(
                        title: Text(
                          "Take ${i + 1} at ${times[i].format(context)}",
                        ),
                        trailing: DropdownButton<String>(
                          value: statusList[i] != "Pending"
                              ? statusList[i]
                              : null,
