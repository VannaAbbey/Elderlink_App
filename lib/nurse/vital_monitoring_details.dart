...
class _VitalDetailScreenState extends State<VitalDetailScreen>
  ...
  Future<void> updateVital(String vitalId) async {
    ...
    final newVitals = {
      ...
      "pulse_rate": int.tryParse(draft.pulseRateController.text) ?? 0,
      "o2_sat": int.tryParse(draft.o2SatController.text) ?? 0,
      "temperature": double.tryParse(draft.temperatureController.text) ?? 0,
      "respiratory_rate": int.tryParse(draft.respiratoryRateController.text) ?? 0,
              ...
              actions: [
                ...
                ElevatedButton(
                  ...
                  style: ElevatedButton.styleFrom(
                    backgroundColor: isChecked ? const Color(0xFF00588E) : Colors.grey,
                  ),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text("Vitals - ${widget.elderlyName}",
      ...
      body: StreamBuilder<QuerySnapshot>(
        ...
        builder: (context, snapshot) {
          ...
          vitals.sort((a, b) {
            final tA = (a['vital_record_at'] as Timestamp?)?.toDate() ?? DateTime(2000);
            final tB = (b['vital_record_at'] as Timestamp?)?.toDate() ?? DateTime(2000);
          ...
          final recordTime = (latestVital['vital_record_at'] as Timestamp?)?.toDate() ?? DateTime.now();
          ...
          return Padding(
            ...
            child: Form(
              ...
              child: ListView(
                children: [
                  _buildTextField("Blood Pressure", draft.bloodPressureController, "e.g. 120/80"),
                  const SizedBox(height: 12),
                  _buildTextField("Pulse Rate", draft.pulseRateController, "e.g. 72", isNumber: true),
                  const SizedBox(height: 12),
                  _buildTextField("O₂ Saturation", draft.o2SatController, "e.g. 97", isNumber: true),
                  const SizedBox(height: 12),
                  _buildTextField("Temperature", draft.temperatureController, "e.g. 36.7", isNumber: true),
                  const SizedBox(height: 12),
                  _buildTextField("Respiratory Rate", draft.respiratoryRateController, "e.g. 18", isNumber: true),
                  const SizedBox(height: 12),
                  _buildTextField("Remarks", draft.remarksController, "Optional"),
                  ...
                  Row(
                    children: [
                      ...
                      Text("Recorded at: ${DateFormat.yMMMd().add_jm().format(recordTime)}"),
                    ],
                  ),
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton(
                      onPressed: !isSaving ? () => updateVital(latestVital['vital_id']) : null,
                      style: ElevatedButton.styleFrom(
                        backgroundColor: !isSaving ? const Color(0xFF00588E) : Colors.grey,
