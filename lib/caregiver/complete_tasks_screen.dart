...
class TaskDetailsDialog extends StatelessWidget {
  ...
  String _formatSingleTime(dynamic dateTime) {
...
class _LabeledField extends StatelessWidget {
  ...
  final String label;
  final String value;
  const _LabeledField({required this.icon, required this.label, required this.value});
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Icon(icon, color: Color(0xFF22688E), size: 20),
          const SizedBox(width: 8),
          Text(
            label,
            style: const TextStyle(fontWeight: FontWeight.bold, color: Color(0xFF22688E)),
          ),
                      ...
                      for (final task in grouped[key]!)
                        GestureDetector(
                          onTap: () {
                            showDialog(
                              context: context,
                              builder: (ctx) => TaskDetailsDialog(task: task),
                            );
                          },
                          child: Card(
